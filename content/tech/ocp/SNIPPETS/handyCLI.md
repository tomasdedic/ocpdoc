...
```sh
 #master yq
	@yq w -i cluster-config.yaml cloud_config \
  	"'$(yq merge \
  	<(yq read cluster-config.yaml cloud_config |sed "s/'{/{/; s/}'/}/" | jq ". | del(.[] | nulls)") \
  	<(oc get cm cloud-provider-config -n openshift-config -o jsonpath="{.data.config}")\
  	-j| jq .)'"
```
